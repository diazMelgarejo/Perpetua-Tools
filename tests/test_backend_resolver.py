...
async def test_resolver_picks_lmstudio_win_for_shared_coding():
    ...
    reg._backends["lmstudio-win"] = Backend(
        "lmstudio-win", "http://127.0.1.2:1234/v1", BackendKind.LMSTUDIO,
    ...
    assert backend.base_url == "http://127.0.1.2:1234/v1"
...
async def test_resolver_honors_base_url_override_matching_known_backend():
    ...
    spec = {"base_url_override": "http://127.0.1.2:1234/v1"}
    backend = resolve_backend_for_spec(reg, spec)
...
async def test_resolver_override_synthesizes_adhoc_backend_for_unknown_url():
    ...
    reg = BackendRegistry()
    spec = {"base_url_override": "http://127.0.1.3:1234/v1"}
    backend = resolve_backend_for_spec(reg, spec)
    assert backend.name == "adhoc"
    assert backend.base_url == "http://127.0.1.3:1234/v1"
...
async def test_resolver_rejects_windows_only_to_mac_mirror():
    ...
    reg._backends["lmstudio-mac"] = Backend(
        "lmstudio-mac", "http://127.0.1.1:1234/v1", BackendKind.LMSTUDIO,
        ("model-x",), BackendHealth.ONLINE, datetime.now(timezone.utc),
    )
    spec = {
        "base_url_override": "http://127.0.1.1:1234/v1",
        "windows_only": True,
    }
