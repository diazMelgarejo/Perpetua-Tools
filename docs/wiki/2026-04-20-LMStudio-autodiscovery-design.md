...
|---|---|---|
| `~/.openclaw/openclaw.json` | `<YOUR_LAN_IP>` | `<YOUR_LAN_IP>` |
| `Perpetua-Tools/config/devices.yml` | `<YOUR_LAN_IP>` | `<YOUR_LAN_IP>` |
| `Perpetua-Tools/.env.example` | `localhost` | `<YOUR_LAN_IP>` |
| `orama-system/.env.example` | `localhost` | `<YOUR_LAN_IP>` |

Live endpoints confirmed: Mac `<YOUR_LAN_IP>:1234` · Win `<YOUR_LAN_IP>:1234`.  
...
scripts/discover-lm-studio.sh   (Layer B — per-repo shell gate)
         ...
         ~/.openclaw/scripts/discover.py   (Layer A — Python hub)
                  ...
                  ├─ Scan <YOUR_LAN_IP>/24:1234   → win_ip, win_models  (async, 200ms)
...
LM_STUDIO_MAC_ENDPOINT=http://<YOUR_LAN_IP>:1234
