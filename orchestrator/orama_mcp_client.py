"""Async MCP client for orama-system stdio JSON-RPC server."""
...
log = logging.getLogger("orchestrator.orama_mcp_client")
...
class OramasysMCPClient:
    """Async MCP client that spawns the orama stdio server and calls oramasys_solve."""
    ...
    async def __aenter__(self) -> "OramasysMCPClient":
        await self._start()
        return self
    ...
    async def call_solve(self, task: str, task_type: str) -> Dict[str, Any]:
        """Call oramasys_solve; raise if result is absent or stub-only."""
...
# Backward-compatible alias for one v1.x release.
UltrathinkMCPClient = OramasysMCPClient
