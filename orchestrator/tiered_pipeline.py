...
4. the configured recipe fits inside repository and approval budgets.

No prompt/output content is persisted to telemetry. Provider credentials are
read from the environment at execution time and never serialized.
"""
...
StageExecutor = Callable[
    [str, str, int, float], Awaitable[tuple[str, int, float]]
]
...
def _require_keys(
    obj: Mapping[str, Any], required: set[str], where: str
) -> None:
...
def _reject_unknown(
    obj: Mapping[str, Any], allowed: set[str], where: str
) -> None:
...
def load_pipeline_config(path: str | Path | None = None) -> dict[str, Any]:
    ...
    models = raw["models"]
    recipes = raw["recipes"]
    if not all(isinstance(value, dict) for value in (limits, models, recipes)):
        raise PipelineConfigError(
            "limits, models, and recipes must be mappings"
        )
    ...
    if limits["max_pipeline_stages"] < 1 or limits["max_total_tokens"] < 1:
        raise PipelineConfigError(
            "pipeline stage/token limits must be positive"
        )
    max_cost = limits["max_total_cost_usd"]
    if not isinstance(max_cost, (int, float)) or max_cost <= 0:
        raise PipelineConfigError(
            "limits.max_total_cost_usd must be positive"
        )

    for model_class, model_cfg in models.items():
        if not isinstance(model_cfg, dict):
            raise PipelineConfigError(
                f"model {model_class} must be a mapping"
            )
        ...
        if model_cfg["provider"] != "openrouter":
            raise PipelineConfigError(
                f"model {model_class} provider must be openrouter"
            )
        slug = model_cfg["model"]
        if not isinstance(slug, str) or "/" not in slug:
            raise PipelineConfigError(
                f"model {model_class} must use an explicit provider/model slug"
            )

    for recipe_id, recipe in recipes.items():
        if not isinstance(recipe, dict) or set(recipe) != {"stages"}:
            raise PipelineConfigError(
                f"recipe {recipe_id} must contain only stages"
            )
        stages = recipe["stages"]
        if not isinstance(stages, list) or not stages:
            raise PipelineConfigError(
                f"recipe {recipe_id} stages must be a non-empty list"
            )
        if len(stages) > limits["max_pipeline_stages"]:
            raise PipelineConfigError(
                f"recipe {recipe_id} exceeds max_pipeline_stages"
            )
        seen: set[str] = set()
        token_sum = 0
        for index, stage in enumerate(stages):
            ...
            if (
                ...
                raise PipelineConfigError(
                    f"{where} has invalid/duplicate name"
                )
            if stage["model_class"] not in models:
                raise PipelineConfigError(
                    f"{where} references unknown model_class"
                )
            if (
                not isinstance(stage["max_tokens"], int)
                or stage["max_tokens"] <= 0
            ):
                raise PipelineConfigError(
                    f"{where}.max_tokens must be positive"
                )
            instruction = stage["instruction"]
            if not isinstance(instruction, str) or not instruction.strip():
                raise PipelineConfigError(
                    f"{where}.instruction must be non-empty"
                )
            inputs = stage["inputs"]
            if (
                ...
                raise PipelineConfigError(
                    f"{where}.inputs must be a non-empty string list"
                )
            allowed_inputs = {"original", *seen}
            if any(source not in allowed_inputs for source in inputs):
                raise PipelineConfigError(
                    f"{where} references an input that is not yet available"
                )
            seen.add(name)
            token_sum += stage["max_tokens"]
        if token_sum > limits["max_total_tokens"]:
            raise PipelineConfigError(
                f"recipe {recipe_id} exceeds max_total_tokens"
            )
    return raw


def _recipe_token_ceiling(
    config: Mapping[str, Any], recipe_id: str
) -> int:
    return sum(
        int(stage["max_tokens"])
    ...
    *,
    ...
    if expiry.tzinfo is None:
        raise PipelinePolicyError(
            "approval expires_at must be timezone-aware"
        )
    if expiry <= now:
        raise PipelinePolicyError("pipeline approval is expired")
    if not all(
        value.strip()
        for value in (approval.trace_id, approval.human_ref, approval.purpose)
    ):
        raise PipelinePolicyError(
            "approval trace_id, human_ref, and purpose are required"
        )
    if approval.recipe_id != recipe_id:
        raise PipelinePolicyError(
            "approval recipe does not match requested recipe"
        )
    if approval.allowed_route_tier != 5:
        raise PipelinePolicyError(
            "approval must explicitly authorize route tier 5"
        )
    if "openrouter" not in approval.provider_scope:
        raise PipelinePolicyError(
            "approval provider scope does not include openrouter"
        )
    if approval.max_tokens < _recipe_token_ceiling(config, recipe_id):
        raise PipelinePolicyError(
            "approval token budget is below the selected recipe ceiling"
        )
    configured_cost = float(config["limits"]["max_total_cost_usd"])
    if approval.max_cost_usd < configured_cost:
        raise PipelinePolicyError(
            "approval cost budget is below the configured pipeline ceiling"
        )
...
def _append_trace(
    trace_path: Path | None, payload: Mapping[str, Any]
) -> None:
...
async def _openrouter_execute(
    ...
    if not api_key:
        raise PipelinePolicyError(
            "OPENROUTER_API_KEY is required when Tier-5 pipelines are enabled"
        )
    ...
    try:
        ...
        raise PipelinePolicyError(
            "OpenRouter response missing content or authoritative usage/cost"
        ) from exc
...
async def execute_tier5_pipeline(
    ...
    *,
    ...
    """Execute one approved Tier-5 recipe behind the canonical gate."""
    if not pipelines_enabled():
        raise PipelinePolicyError(
            "Tier-5 pipelines are disabled; "
            "set PIPELINE_TIERED_ENABLED=1 explicitly"
        )
    ...
    if not gate.has_route:
        raise PipelinePolicyError(
            gate.denied_reason
            or "canonical frugality gate denied Tier 5"
        )

    config = load_pipeline_config(config_path)
    if recipe_id not in config["recipes"]:
        raise PipelineConfigError(
            f"unknown pipeline recipe: {recipe_id}"
        )
    _validate_approval(
        approval,
        recipe_id=recipe_id,
    ...
    max_tokens = min(
        _recipe_token_ceiling(config, recipe_id), approval.max_tokens
    )
    max_cost = min(
        float(limits["max_total_cost_usd"]), approval.max_cost_usd
    )
    ...
    for stage in config["recipes"][recipe_id]["stages"]:
        ...
        prompt = _build_prompt(
            stage["instruction"], stage["inputs"], values
        )
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt >= max_retries or not _retryable_transport_error(exc):
                    raise PipelinePolicyError(
                        "Tier-5 provider request failed after governed retry policy"
                    ) from exc
                await asyncio.sleep(0)

        if used_tokens < 0 or cost_usd < 0:
            raise PipelinePolicyError(
                "provider returned invalid negative usage"
            )
        cumulative_tokens += used_tokens
        cumulative_cost += cost_usd
        if cumulative_tokens > max_tokens:
            raise PipelinePolicyError(
                "Tier-5 pipeline exceeded approved/configured token budget"
            )
        if cumulative_cost > max_cost:
            raise PipelinePolicyError(
                "Tier-5 pipeline exceeded approved/configured cost budget"
            )
